...
const CREATOR = { name: "Macdonald Barasa", role: "Creator of Voice Orb" };
  ...
  return (
    <div className="relative w-full min-h-[100dvh] overflow-hidden select-none" style={{ background: "var(--bg)", color: "var(--text)" }}>
      ...
      <Overlay open={settingsOpen} onClose={()=>setSettingsOpen(false)} bottom>
        <div className="mx-auto max-w-[560px] px-6 pt-3 pb-[max(20px,env(safe-area-inset-bottom))]"><div className="flex justify-center pt-1 pb-4"><div className="w-9 h-1 rounded-full bg-black/10"/></div><div className="flex items-center justify-between mb-6"><h3 className="text-lg font-semibold">Settings</h3><button onClick={()=>setSettingsOpen(false)} className="w-8 h-8 rounded-full flex items-center justify-center" style={{background:"var(--accent-soft)"}}><X className="w-4 h-4"/></button></div>
