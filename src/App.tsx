...
export default function App() {
  ...
  const [orbStyle, setOrbStyle] = useState<"classic" | "particle">("particle");
...
<div className="mt-3 p-4 rounded-2xl border flex items-center justify-between" style={{background:"var(--surface)",borderColor:"var(--border)"}}>
  ...
  <button onClick={currentUser ? logout : signIn} className="px-4 py-2 rounded-full text-xs font-semibold border" style={{background:currentUser?"var(--surface)":"var(--text)",color:currentUser?"var(--text)":"var(--bg)",borderColor:"var(--border)"}}>
    {currentUser ? "Sign Out" : "Sign in with Google"}
            ...
            <section><label className="text-xs font-semibold tracking-wide uppercase" style={{color:"var(--muted)"}}>Orb</label><div className="mt-3 flex items-center gap-3 rounded-2xl border p-3" style={{background:"rgba(255,166,91,.10)",borderColor:"rgba(232,121,40,.35)"}}><span className="h-10 w-10 shrink-0 rounded-full" style={{background:"radial-gradient(circle, #fff8ef 0 18%, #ffc28e 42%, #e87928 72%, transparent 73%)",boxShadow:"0 0 22px rgba(232,121,40,.35)"}}/><div><p className="text-sm font-semibold">Orange particle orb</p><p className="text-xs mt-1" style={{color:"var(--muted)"}}>The single Neto orb experience.</p></div></div></section>
